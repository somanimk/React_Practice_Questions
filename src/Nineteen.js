...
const Nineteen = () => {
    ...
    const addToCart = (item) => {
        ...
        if(existingCartItem) {
            ...
            cartDispatch({ type: 'ADD_TO_CART', payload: { ...item, quantity: 1 } }); // Fixed here: payload should be lowercase
        }
    }

    const updateQuantity = (itemId, quantity) => {
        if(quantity > 0) {
            cartDispatch({ type: 'UPDATE_QUANTITY', payload: { id: itemId, quantity: quantity } });
        }
    }
